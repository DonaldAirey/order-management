namespace Teraque
{
	...
	class SchemaScrubber
	{
		...
		private static XNamespace tdata;
		...
		static SchemaScrubber()
		{
			...
			SchemaScrubber.msprop = "urn:schemas-microsoft-com:xml-msprop";
			SchemaScrubber.tdata = "urn:schemas-teraque-com:xml-tdata";
		...
		private static XElement CreateTable(TableSchema tableSchema)
		{
			...
			// Preserve the 'isPersistent' flag when the table is not persistent.
			if (!tableSchema.IsPersistent)
				tableElement.SetAttributeValue(SchemaScrubber.tdata + "isPersistent", false);
